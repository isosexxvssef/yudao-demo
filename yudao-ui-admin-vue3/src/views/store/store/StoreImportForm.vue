<template>
  <Dialog v-model="dialogVisible" title="门店导入" width="400">
    <el-upload
      ref="uploadRef"
...
defineOptions({ name: 'StoreImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/store/store/import'
const uploadHeaders = ref() // 上传 Header 头
const fileList = ref([]) // 文件列表
const updateSupport = ref(0) // 是否更新已经存在的门店数据
...
const importTemplate = async () => {
  const res = await StoreApi.importTemplate()
  download.excel(res, '门店导入模板.xls')
}
</script>
